...
export function registerAsset(registry, asset, usage) {
  const normalized = { ...asset, contentHash: asset.contentHash || contentHash(asset.imageUrl || asset.sourceUrl || asset.externalId || asset.prompt), usage: [] };
  ...
  if (existing) {
    existing.usage ??= [];
    if (usage && !existing.usage.some((entry) => entry.projectId === usage.projectId && entry.sceneId === usage.sceneId)) existing.usage.push({ ...usage, usedAt: usage.usedAt || new Date().toISOString() });
...
export function registrySummary(registry) {
  return {
    ...
    generated: registry.filter(asset => asset.sourceType === 'generated').length,
    stock: registry.filter(asset => asset.sourceType === 'stock').length,
